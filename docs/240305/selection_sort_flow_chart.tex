...
\begin{tikzpicture}
    ...
    \node[draw, io, below= of a] (b) {Liste (unsortiert)};
    \node[draw, proc, below= of b] (c) {kleinstes Element suchen};
    \node[draw, proc, below= of c] (d) {kleinstes Element nach Vorne };
    \node[draw, proc, below= of d] (e) {Liste kürzen};
    \node[draw, test, below= of e] (f) {len(Liste) > 1};
    \node[draw, io, below= of f] (g) {Liste (sortiert)};
